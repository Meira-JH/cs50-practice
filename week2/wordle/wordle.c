...
int main(int argc, string argv[]) {
  ...
  // instead
  // TODO #2
  if (strlen(argv[1]) > 1 || (int)argv[1][0] < 52 || (int)argv[1][0] > 54) {
  ...
  for (int i = 0; i < guesses; i++) {
    ...
    // TODO #4
    if (i == 0) {
      for (int k = 0, length = wordsize; k < length; k++) {
        status[k] = 0;
      }
    }
...
int check_word(string guess, int wordsize, int status[], string choice) {
  ...
  // status
  // TODO #5
  for (int i = 0, firsLoopLength = wordsize; i < firsLoopLength; i++) {
    for (int j = 0, secondLoopLength = wordsize; j < secondLoopLength; j++) {
  ...
  // break so you don't compare that letter further if it's in the word, but not
  // the right spot, score CLOSE point (yellow) keep track of the total score by
  // adding each individual letter's score from above
...
void print_word(string guess, int wordsize, int status[]) {
  ...
  for (int i = 0, length = wordsize; i < length; i++) {
    if (status[i] == 0) {
